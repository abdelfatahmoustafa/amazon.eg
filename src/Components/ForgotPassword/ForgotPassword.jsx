...
import logo_amazon_auth from "../../images/logo-amazon-auth.png";
import { useFormik } from "formik";
import * as Yup from "yup";
import axios from "axios";
import { useNavigate } from "react-router-dom";
import loading from "../../images/loading-amazon.gif";
export default function Login() {
  ...
  const validationSchemaCode = Yup.object({
    resetCode: Yup.string()
      ...
      .max(6, "It Must be Maxmum 6 Numbers"),
  });

  async function handelLogin(values) {
    setLoading(true);
    const { data } = await axios
      .post("${BASE_API}/api/v1/auth/forgotPasswords", values)
      .catch((error) => {
        setLoading(false);
  ...
  async function handelCode(values) {
    setLoading(true);
    const { data } = await axios
      .post("${BASE_API}/api/v1/auth/verifyResetCode", {
            ...
            <div className="d-flex justify-content-center align-items-center">
              <div className="w-50 bg-white rounded-3 my-5 shadow">
                <div className="w-25 my-3 mx-auto">
                  <img
                    src={logo_amazon_auth}
                    className="w-100 text-center"
                    alt="  logo_amazon_auth"
                  />
                </div>
